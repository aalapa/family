...
export class EditHabitModal extends LitElement {
    static styles = css`
        :host {
            position: fixed;
            top: 0;
    ...
    open(habitName) {
        const habit = this.existingHabits.find(h => h.person === this.currentUser && h.habit === habitName);
        if (!habit) {
            this.dispatchEvent(new CustomEvent('show-notification', {
                detail: { message: 'Habit not found!', type: 'error' },
                bubbles: true
            }));
            return;
        }
        
        this.originalHabitName = habitName;
        this.habitName = habit.habit;
        this.habitCategory = habit.category;
        this.habitSchedule = habit.schedule;
    ...
    _confirmEdit() {
        const newName = this.habitName.trim();
        const newCategory = this.habitCategory.trim();
        ...
        // Check if habit name already exists (excluding current habit)
        const existingHabit = this.existingHabits.find(h => 
            h.person === this.currentUser && 
            h.habit.toLowerCase() === newName.toLowerCase() &&
            h.habit !== this.originalHabitName
        );
        ...
        this.dispatchEvent(new CustomEvent('edit-habit', {
            detail: { 
                originalHabitName: this.originalHabitName,
                newName, 
                newCategory, 
                newSchedule: this.habitSchedule 
            },
            bubbles: true
        }));
...
customElements.define('edit-habit-modal', EditHabitModal); 